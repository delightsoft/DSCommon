 ...
 */
public final class DateBinder extends JsonTypeBinder.FieldAccessor {

    public DateBinder(Field fld, Method getter, Method setter, String fldName) {
    ...
    public void copyFromJson(final Object obj, final JsonNode node,
        ...
        final LocalDate t = (LocalDate) getter.invoke(obj);
        final LocalDate vDate = (v == null) ? null : new LocalDate(v);
            ...
            if (update != null) {
                ...
                if (!derived) {
                    if (update.undoNode != null && !update.undoNode.has(fldName))
                        ...
                        else update.undoNode.put(fldName, t.toDateTime(LocalTime.MIDNIGHT).getMillis());
    ...
    public void copyToJson(final Object obj, Template template, final ObjectNode out, Stack<String> stack, int mode, DocumentAccessActionsRights rights, BitArray mask)
            throws Exception {
        final LocalDate v = (LocalDate) getter.invoke(obj);
        if (v == null) out.putNull(fldName);
        else out.put(fldName, v.toDateMidnight(DateTimeZone.UTC).getMillis());
    }
}
