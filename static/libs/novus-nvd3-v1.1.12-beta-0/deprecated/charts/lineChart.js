...
//    basically the chart docs with the extra glue... Queuing, tooltips, automatic resize, etc.
...
nv.charts.line = function() {
  ...
  //TODO: consider a method more similar to how the docs are built
  function chart() {
    if (!selector || !data.length) return chart; //do nothing if you have nothing to work with

    d3.select(selector).select('svg')
        .datum(data)
      .transition().duration(duration).call(graph); //consider using transition chaining like in the docs
